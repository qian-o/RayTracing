...
public class Chapter5 : IChapter
{
    public void Run()
    {
        ...
        for (int i = 0; i < image_height; i++)
        {
            ...
            for (int j = 0; j < image_width; j++)
            {
                ...
                Vector3D<double> pixel_color = RayColor(ray);
    ...
    private static Vector3D<double> RayColor(Ray ray)
    {
        if (HitSphere(new Vector3D<double>(0.0, 0.0, -1.0), 0.5, ray))
        {
            return new Vector3D<double>(1.0, 0.0, 0.0);
        }
    ...
    private static bool HitSphere(Vector3D<double> center, double radius, Ray ray)
    {
        Vector3D<double> oc = center - ray.Origin;
        double a = Vector3D.Dot(ray.Direction, ray.Direction);
        double b = -2.0 * Vector3D.Dot(ray.Direction, oc);
