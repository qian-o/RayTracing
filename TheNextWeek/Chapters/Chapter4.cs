...
public class Chapter4 : IChapter
{
    public void Run()
    {
        int index = 3;
    ...
    private static void RandomSpheres()
    {
        HittableList world = new();

        Lambertian ground_material = new(new CheckerTexture(new Vector3D<double>(0.2, 0.3, 0.1), new Vector3D<double>(0.9, 0.9, 0.9), 0.32));
    ...
    private static void TwoSpheres()
    {
        ...
        CheckerTexture checker = new(new Vector3D<double>(0.2, 0.3, 0.1), new Vector3D<double>(0.9, 0.9, 0.9), 0.8);

        world.Add(new Sphere(new Vector3D<double>(0.0, -10.0, 0.0), 10.0, new Lambertian(checker)));
        world.Add(new Sphere(new Vector3D<double>(0.0, 10.0, 0.0), 10.0, new Lambertian(checker)));
